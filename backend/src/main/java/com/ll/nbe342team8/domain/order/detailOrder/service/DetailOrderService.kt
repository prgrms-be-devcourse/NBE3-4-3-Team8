...
class DetailOrderService(
    ...
    fun getDetailOrdersByOrderIdAndMember(orderId: Long, member: Member): List<DetailOrderDto> {
        ...
        val detailOrders = detailOrderRepository.findByOrderId(orderId)
        println("detailOrders = $detailOrders")
        // 주문 상세 정보를 DetailOrderDto로 변환하여 반환
        return detailOrders.map { detailOrder ->
            DetailOrderDto(
                orderId = detailOrder.order.id ?: throw IllegalArgumentException("Order ID is null"),
                bookId = detailOrder.book.id ?: throw IllegalArgumentException("Book ID is null"),
